...
func (p *PullOrderApp) Start(ctx context.Context) error {
	...
	// 注册章鱼平台到管理器
	zhangyuPlatform := pullorder.NewZhangyuPullPlatform(p.platformAccountRepo, p.variantRepo)
	p.mgr.RegisterPlatform(zhangyuPlatform)
	// 构造调度器（注入统一配置仓库）
	// 调度器保持原构造签名，统一按变体处理；pullCfgRepo 不再用于章鱼
	p.scheduler = pullorder.NewPullOrderScheduler(p.mgr, p.platformAccountRepo, p.variantRepo, nil)
