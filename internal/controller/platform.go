...
func (c *PlatformController) UpdatePlatform(ctx *gin.Context) {
	...
	if err != nil {
		resp.Error(ctx, http.StatusBadRequest, "invalid platform id")
		return
	}
...
func (c *PlatformController) ListPlatformAccounts(ctx *gin.Context) {
	...
	data, err := c.service.ListPlatformAccounts(&req)
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, err.Error())
		return
	}
...
func (c *PlatformController) UpdatePlatformAccount(ctx *gin.Context) {
	id, err := strconv.ParseInt(ctx.Param("id"), 10, 64)
	if err != nil {
		resp.Error(ctx, http.StatusBadRequest, "invalid account id")
		return
	}
...
func (c *PlatformController) GetChannelList(ctx *gin.Context) {
	...
	if name != "" {
		...
		if err != nil {
			resp.Error(ctx, http.StatusBadRequest, fmt.Sprintf("未找到账号名为 %s 的平台账号: %v", name, err))
			return
		}
		...
		if err != nil {
			resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("获取平台信息失败: %v", err))
			return
		}
...
func (c *PlatformController) GetBeeProductList(ctx *gin.Context) {
	...
	if err != nil {
		resp.Error(ctx, http.StatusNotFound, "平台账号不存在")
		return
	}
	...
	if err != nil || platform.Code != "mifeng" {
		resp.Error(ctx, http.StatusBadRequest, "该账号不是蜜蜂平台账号")
		return
	}
	...
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("获取商品列表失败: %v", err))
		return
	}
	...
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("解析商品数据失败: %v", err))
		return
	}
...
func (c *PlatformController) UpdateBeeProductPrice(ctx *gin.Context) {
	...
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("更新商品价格失败: %v", err))
		return
	}
...
func (c *PlatformController) UpdateBeeProductProvince(ctx *gin.Context) {
	...
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("更新商品省份配置失败: %v", err))
		return
	}
...
func (c *PlatformController) EditBeeSupplyGoodManageStock(ctx *gin.Context) {
	...
	// 调用蜜蜂平台API
	data, err := c.beeService.EditSupplyGoodManageStock(account, &req)
	if err != nil {
		resp.Error(ctx, http.StatusInternalServerError, fmt.Sprintf("修改商品报价失败: %v", err))
		return
	}
