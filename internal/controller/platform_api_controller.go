...
func (c *PlatformAPIController) CreateAPI(ctx *gin.Context) {
	...
	// 参数验证
	if api.PlatformID == 0 {
		resp.Error(ctx, 400, "平台ID不能为空")
		return
	}
	if api.Name == "" {
		resp.Error(ctx, 400, "接口名称不能为空")
		return
	}

	if api.URL == "" {
		resp.Error(ctx, 400, "接口地址不能为空")
		return
	}
	...
	if err != nil {
		...
		resp.Error(ctx, 500, "获取平台信息失败")
		return
	}
	if platform == nil {
		resp.Error(ctx, 400, "平台不存在")
		return
	}
