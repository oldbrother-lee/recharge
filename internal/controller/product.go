...
func (c *ProductController) List(ctx *gin.Context) {
	...
	log.WithContextCategory(ctx.Request.Context(), "product_controller").Info("绑定参数成功",
		log.IntV2("page", req.Page),
		log.IntV2("page_size", req.PageSize),
		log.IntV2("type", req.Type),
		log.IntV2("category", req.Category),
		log.StringV2("isp", req.ISP),
