...
func (a *AuthMiddleware) Auth() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if authHeader == "" {
            resp.Error(ctx, 401, "Authorization header is required")
			ctx.Abort()
			return
		}
		...
		if !(len(parts) == 2 && parts[0] == "Bearer") {
            resp.Error(ctx, 401, "Authorization header format must be Bearer {token}")
			ctx.Abort()
			return
		}
		...
		if err != nil {
            resp.Error(ctx, 401, "Invalid token")
			ctx.Abort()
			return
		}
		...
		if err != nil {
            resp.Error(ctx, 401, "User not found")
			ctx.Abort()
			return
		}
		...
		// 检查用户是否被禁用
		if user.Status != 1 {
            resp.Error(ctx, 401, "Account has been disabled")
			ctx.Abort()
			return
		}
...
func Auth() gin.HandlerFunc {
	if globalAuthMiddleware == nil {
        ...
        return func(ctx *gin.Context) {
            resp.Error(ctx, 500, "Authentication middleware not initialized")
            ctx.Abort()
        }
	}
