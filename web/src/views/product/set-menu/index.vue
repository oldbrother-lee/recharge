...
import { type Interface, useInterfaceStore } from '@/store/modules/interface';
...
const handleEditRelation = (row: any) => {
  ...
  const sameChannelEnabled =
    scrEnabledRaw === true || scrEnabledRaw === 1 || scrEnabledRaw === '1' || scrEnabledRaw === 'true';
  ...
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard title="商品管理" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      <template #header-extra>
        <NSpace>
          <NButton v-if="hasRole('SUPER_ADMIN')" type="primary" @click="showCategoryModal">分类管理</NButton>
          <NButton v-if="hasRole('SUPER_ADMIN')" type="primary" @click="handleAdd()">新增商品</NButton>
        </NSpace>
      </template>
      <NDataTable
        :columns="columns"
      ...
      >
        <NGrid :cols="24" :x-gap="24">
          ...
          <NFormItemGi :span="12" label="显示端" path="show_style">
            <NSelect v-model:value="formModel.show_style" :options="SHOW_STYLE_OPTIONS" placeholder="请选择显示端" />
          </NFormItemGi>
          ...
          <NFormItemGi :span="12" label="封顶价格" path="max_price">
            <NInputNumber
              v-model:value="formModel.max_price"
              :precision="2"
              :step="0.1"
              :min="0"
              :show-button="false"
    ...
    <!-- 接口选择对话框 -->
    <NModal v-model:show="showInterfaceDialog" preset="dialog" title="选择接口" :style="{ width: '1000px' }">
      <div class="flex flex-col gap-16px">
        <!-- 工具栏 -->
        <div class="flex justify-end">
          <NButton type="primary" @click="handleOpenCreateRelation">创建关联</NButton>
        </div>
      </div>
    ...
    >
      ...
      >
        <NFormItem label="接口" path="api_id">
          <NSelect v-model:value="relationForm.api_id" :options="interfaceOptions" placeholder="请选择接口" />
        </NFormItem>
        <NFormItem label="选择套餐" path="param_id">
          <NSelect v-model:value="relationForm.param_id" :options="packageOptions" placeholder="请选择套餐" />
        </NFormItem>
