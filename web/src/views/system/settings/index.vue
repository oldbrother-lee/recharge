...
import { NButton, NCard, NForm, NFormItem, NInput, NInputNumber, NSwitch, NTag, useMessage } from 'naive-ui';
...
const loadSystemConfig = async () => {
  try {
    ...
    if (response.data) {
      ...
      configForm.sessionTimeout = Number.parseInt(configs.session_timeout) || 3600;
      configForm.maxUploadSize = Math.round((Number.parseInt(configs.max_upload_size) || 10485760) / 1024 / 1024);
  ...
  try {
    ...
    if (resp.code === 0) {
      ...
      window.dispatchEvent(
        new CustomEvent('system-logo-updated', {
...
<template>
  <div class="system-settings">
    ...
    <div class="settings-container">
      <!-- 基本信息设置 -->
      <NCard title="基本信息" class="settings-card">
        ...
        >
          <NFormItem label="系统名称" path="systemName">
            <NInput v-model:value="basicForm.systemName" placeholder="请输入系统名称" :maxlength="50" show-count />
          </NFormItem>
          <NFormItem label="系统版本" path="systemVersion">
            <NInput v-model:value="basicForm.systemVersion" placeholder="请输入系统版本" :maxlength="20" show-count />
          </NFormItem>
          <NFormItem label="系统描述" path="systemDescription">
            <NInput
          ...
          <NFormItem label="系统Logo" path="systemLogo">
            <!-- 调试信息 -->
            <div
              style="
                margin-bottom: 10px;
              ...
              "
            >
              调试信息: basicForm.systemLogo =
              {{ basicForm.systemLogo ? '有值 (长度: ' + basicForm.systemLogo.length + ')' : '无值' }}
            </div>
            <div class="logo-container">
              <div v-if="basicForm.systemLogo" class="logo-preview">
                <img :src="basicForm.systemLogo" alt="系统Logo" class="logo-image" />
                <div class="logo-actions">
                  <NButton size="small" @click="handleLogoPreview">预览</NButton>
                  <NButton size="small" type="error" @click="handleLogoRemove">删除</NButton>
                </div>
              </div>
              <div v-else class="logo-upload">
                ...
                >
                  <NUploadDragger>
                    ...
                    <NP depth="3" style="margin: 8px 0 0 0">支持 JPG、PNG、GIF 格式，文件大小不超过 2MB</NP>
                  </NUploadDragger>
                </NUpload>
              </div>
            </div>
          ...
          <NFormItem>
            <NButton type="primary" :loading="basicLoading" @click="handleBasicSubmit">保存基本信息</NButton>
          </NFormItem>
        </NForm>
      </NCard>

      <!-- 系统配置 -->
      <NCard title="系统配置" class="settings-card">
        <template #header-extra>
        ...
        >
          <NFormItem label="维护模式" path="maintenanceMode">
            <NSwitch v-model:value="configForm.maintenanceMode" checked-value="true" unchecked-value="false">
          ...
          <NFormItem label="最大上传大小" path="maxUploadSize">
            <NInputNumber v-model:value="configForm.maxUploadSize" :min="1" :max="100" :step="1" style="width: 200px">
              <template #suffix>MB</template>
            </NInputNumber>
            <div class="form-help">单个文件最大上传大小，范围：1-100MB</div>
          </NFormItem>
          <NFormItem>
            <NButton type="primary" :loading="configLoading" @click="handleConfigSubmit">保存系统配置</NButton>
          </NFormItem>
        </NForm>
      </NCard>

      <!-- 系统信息 -->
      <NCard title="系统信息" class="settings-card">
        <template #header-extra>
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <NTag :type="getStatusType(systemInfo.status)" size="small">
