...
const rules = {
  ...
  style: [{ required: true, message: '请选择扣款类型', trigger: 'change', type: 'number' }]
};
...
<template>
  <NModal v-model:show="visible" preset="dialog" title="余额扣款" :style="{ width: '500px' }">
    <NForm
      ref="formRef"
      :model="formModel"
      :rules="rules"
    ...
    >
      ...
      <NFormItem label="扣款金额" path="amount">
        <NInputNumber v-model:value="formModel.amount" :min="0.01" :precision="2" placeholder="请输入扣款金额" />
      </NFormItem>
      <NFormItem label="扣款类型" path="style">
        <NSelect
